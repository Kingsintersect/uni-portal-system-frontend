...
const UpdateCourseAssignment = ({
   ...
   const {
      ...
      resolver: zodResolver(UpdateCourseAssignmentSchema),
      defaultValues: {
   ...
   const resetRowsAndAvailableCourses = useCallback((newData: Course[]) => {
      setRows([]);
   ...
   const deleteAssignmentApi = async (courseCategoryId: number, courseId: number, creditLoad: number): Promise<boolean> => {
      let deleted = false;
      const { success, error } = await DeleteSingleAssignment(access_token, courseCategoryId, courseId, creditLoad);
      if (error) {
   ...
   }, [courses, courseAssingnments, resetRowsAndAvailableCourses]);

   useEffect(() => {
      if (courseAssingnments) {
         reset({
            course_category_id: String(selectedCategoryId ?? "")
         });
   ...
   const onSubmit: SubmitHandler<CourseAssignmentFormData> = async (data) => {
      ...
      const { error, success } = await UpdateSingleCourseAssignment(courseAssingnments.id, access_token, data);
      if (error) {
         const errorMessages = extractErrorMessages(error);
         errorMessages.forEach((msg) => {
            notify({ message: msg, variant: "error", timeout: 10000 });
...
type CourseAssignmentFormData = z.infer<typeof UpdateCourseAssignmentSchema>;